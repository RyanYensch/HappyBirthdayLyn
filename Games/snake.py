...
class SnakeGame():
    ...
    def game_start(self):
        ...
        self.score_frame = Frame(self.game_window, bg=HEADER_COLOUR, width=GAME_WIDTH)
        self.score_frame.pack(fill='x', expand=True)

        self.label = Label(self.score_frame, text="Score: {}".format(self.score), font=("consolas", 20, "bold"), bg=HEADER_COLOUR)
        self.label.pack(side="left", pady=10, padx=10)
        
        back_button = Button(self.score_frame, text="Back to Menu", command=self.game_window.destroy)
        ...
        self.game_window.bind('<a>', lambda event: self.change_direction('left'))
        self.game_window.bind('<d>', lambda event: self.change_direction('right'))
        self.game_window.bind('<w>', lambda event: self.change_direction('up'))
        self.game_window.bind('<s>', lambda event: self.change_direction('down'))
